...
func getNumber() int {
	...
	fmt.Print("Enter the number of processes to be generated: ")
	...
	if err != nil {
		log.Fatalf("Invalid input, please enter a valid number: %v\n", err)
	}
...
func main() {
	...
	number := getNumber()
	fmt.Printf("You entered number of process to be generated: %s\n", number)
	...
	for i := 0; i < number; i++ {
		...
		// Increment the process name for the next iteration
		processName = fmt.Sprintf("napa%d", i+1) // Format as napa, napa1, napa2, ...
	}
