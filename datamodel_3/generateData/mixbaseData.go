...
func getMeasureIDsForProcess(db *sql.DB, process string) (status string, duration string, results string, errMsg string) {
	...
	for rows.Next() {
		...
		//fmt.Printf("MeasureID: %s", measureID)
		count++
	}
	...
	log.Println("before exiting measureid for process")
...
func queryHandler(c echo.Context) error {
	...
	switch req.Type {
	case "measures":
		log.Println("before get measure query")
		status, timeTaken, results, errMsg = getMeasureIDsForProcess(db, req.Proc)
	// case "results":
